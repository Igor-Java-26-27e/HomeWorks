...
public class ProcessBigArray {
    ...
    public static void main(String[] args) {
        ...
        processArray(array); //Abrabotka Maiva v odin patok
        System.out.println("One thread: " + (System.currentTimeMillis() - startTime) + " ms.");

        startTime = System.currentTimeMillis();
        processArrayTwoThread(array2thread); //Abrabotka Maiva v dva patoka i dve ciasti
        System.out.println("Two thread two parts: " + (System.currentTimeMillis() - startTime) + " ms.");

        startTime = System.currentTimeMillis();
        processArrayTwoThreadWithoutSplitting(array2tws); //Abrabotka Maiva v dva patoka i odna ciasti
    ...
    static void processArray(float[] array) {  // Metod katorii abrabativaet masiv v odin patok
        Arrays.fill(array, 1);
        processArray(array, 0); // Vizivaem peregrujenii Metod processArra y 1
    }

    static void processArray(float[] array, int shift) {  // Peregrujenii Metod processArray 1
        IntStream.range(0, array.length)
                .forEach(i -> array[i] = (float) (array[i]   // Zapalneaem elementi pomashiu Metoda .forEach
                        * Math.sin(0.2 + (i + shift) / 5f) * Math.cos(0.2f + (i + shift) / 5f) * Math.cos(0.4f + (i + shift) / 2f)));
    }
